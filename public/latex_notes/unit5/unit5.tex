...
\begin{equation*}
 \E[Y|X_1,X_2 = x+1] - \E[Y|X_1,X_2 = x] = \beta_2 + J_{1,2}X_1
\end{equation*}
...
\end{exercise}
%---------------------------------------------------------------------------------
\begin{exercise}[Marginal regression in interactions model \ding{111}]
...
%----------------------------------------------------------------------------------------------------------------
\begin{exercise}[Mean-squared error \ding{111}]
Prove formula Eq. \ref{eq:EXhaty}. 
\end{exercise}
%----------------------------------------------------------------------------------------------------------------
\begin{exercise}[Laplace's rule \ding{111}]
...
\begin{enumerate}[label=(\alph*)]
\item \ding{111} Are $\phi_i$ orthogonal with respect to a random variable $X$ taking values in $[0,1)$? Does it depend on the distribution of $X$? 
...
\item \ding{111} As usual let $\hat{\beta}_j$ be the fitted value of $\beta_j$ using least squares, meaning the value that minimizes the squared residuals. Show that in this model $\hat{\beta}_j$ is simply the average value of $Y_i$ among data points where $X_i \in I_j$; that is, 
\begin{equation*}
...
\textbf{Unit 5 Extra Practice}  
\paragraph{}
Consider a regression model where the test scores of students are predicted based on the number of hours studied (\(X_1\)) and their sleep quality score (\(X_2\)).The model is as follows:
...
Where:  
 ...
 \(X_2\) is the sleep quality score (on a scale of 1 to 10),  
 \(X_1 X_2\) is the interaction term between hours studied and sleep quality.
\paragraph{}
 
\begin{enumerate}
  \item[(a)] What are the slopes of \( E[Y | X_1, X_2 = 3] \) vs. \( X_1 \) and \( E[Y | X_1, X_2 = 7] \) vs. \( X_1 \)?  
  \item[(b)] Interpret the interaction term \( \beta_3 \) in this model.
\end{enumerate}
